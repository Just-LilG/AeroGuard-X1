/*
  ...
  Product behaviour (v1):
   ...
   - ESP32 WiFi bridge on A1/A3 (remote app access)
  ============================================================
*/
...
const int PIN_APP_RX      = A1;
const int PIN_FLAME       = A2;
const int PIN_APP_TX      = A3;
...
SoftwareSerial appSerial(PIN_APP_RX, PIN_APP_TX);
...
void setup() {
  ...
  if (SD.begin(PIN_SD_CS)) { sdReady = true; Serial.println(F("SD ready")); logEvent("SYSTEM", "boot"); }
...
void readSensorsAndEvaluate() {
  ...
  if (instant != currentLevel) {
    if (millis() - thresholdCrossedAt >= CONFIRM_WINDOW_MS) setLevel(instant, false);
  }
}
