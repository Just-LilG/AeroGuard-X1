...
| 11 | ESP32 DevKit | **Have** | A1 / A3 + own USB |
...
Then flash [`aeroguard_x1-1.ino`](aeroguard_x1-1.ino) (9600) and [`esp32_aeroguard_bridge.ino`](esp32_aeroguard_bridge.ino) (115200). Set WiFi in the ESP32 sketch.

**Smoke test:** LCD shows Safe after ~45 s calibration. Demo button: Low → Medium → Critical → Fire. Reset recalibrates. Serial `STATUS` lines; ESP32 `http://<ip>/status`.
...
4. SIM800L TX → Uno **D5**.  
5. Uno **D6** → 10kΩ → junction → SIM800L RX; junction → 20kΩ → GND.  
...
| File | Role |
|------|------|
| [`AeroGuard-X1_Pin_Map_Wiring_Reference.md`](AeroGuard-X1_Pin_Map_Wiring_Reference.md) | Every module pin for the current pack |
